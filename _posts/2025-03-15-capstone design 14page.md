...
# 세 자리 숫자 게임: 이론적 기댓값 계산

이 문서에서는 주사위 3개를 사용하여 세 자리 숫자를 만드는 게임에서,  
고정 전략과 남은 두 주사위의 최적 배치를 고려한 경우의 수를 직접 계산하여  
전체 216가지 경우의 이론적 기댓값을 구하는 과정을 설명합니다.

## 1. 전략 요약

**고정 전략:**  
- **그룹 1:** \(d1 \in \{1,2\}\) → \(d1\)은 **일의 자리**에 배치  
  - 남은 두 주사위 \(d2, d3\)는 다음과 같이 배치:  
    - if \(d2 \le 3\): \(d2\)는 **십의 자리**, \(d3\)는 **백의 자리**  
    - if \(d2 \ge 4\): \(d2\)는 **백의 자리**, \(d3\)는 **십의 자리**

- **그룹 2:** \(d1 \in \{3,4\}\) → \(d1\)은 **십의 자리**에 배치  
  - 남은 두 주사위 \(d2, d3\)는 다음과 같이 배치:  
    - if \(d2 \le 3\): \(d2\)는 **일의 자리**, \(d3\)는 **백의 자리**  
    - if \(d2 \ge 4\): \(d2\)는 **백의 자리**, \(d3\)는 **일의 자리**

- **그룹 3:** \(d1 \in \{5,6\}\) → \(d1\)은 **백의 자리**에 배치  
  - 남은 두 주사위 \(d2, d3\)는 다음과 같이 배치:  
    - if \(d2 \le 3\): \(d2\)는 **일의 자리**, \(d3\)는 **십의 자리**  
    - if \(d2 \ge 4\): \(d2\)는 **십의 자리**, \(d3\)는 **일의 자리**

이 결정은 \(d3\)가 1부터 6까지 균등하게 나올 때의 평균(기댓값)을 고려하여  
각 옵션의 최종 세 자리 수의 평균을 비교한 결과입니다.

## 2. 각 그룹별 계산

각 그룹에서 \(d2\)는 1부터 6까지 균등하게 나온다고 가정합니다.  
아래는 각 그룹별로 \(d1\)의 값에 따라 \(d2\)와 \(d3\)의 배치에 따른 최종 세 자리 수를 계산한 과정입니다.
...
- **\(d1 = 1\):**  
  ...
  - **\(d1 = 1\) 총합:** \(1113 + 1608 = 2721\)  
    **평균:** \(2721 / 6 \approx 453.5\).

- **\(d1 = 2\):**  
  비슷하게 계산하여 평균 \(\approx 454.5\).

- **그룹 1 평균:**  
  \[
  \frac{453.5 + 454.5}{2} = 454.0.
  \]
  사건 수: \(2 \times 36 = 72\)  
  **총합 그룹 1:** \(454.0 \times 72 = 32,\!688\).
...
- **\(d1 = 3\):**  
  - **옵션 A (if \(d2 \le 3\)):**  
    최종 수:  
    \[
    f = 350 + 10 \times 3 + d2 = 380 + d2.
    \]
    예: \(d2 = 1,2,3 \rightarrow\) 381, 382, 383;  
    **합계:** 1146.
    
  - **옵션 B (if \(d2 \ge 4\)):**  
    최종 수:  
    \[
    f = 100 \times d2 + 30 + 3.5 = 100 \times d2 + 33.5.
    \]
    예: \(d2 = 4,5,6 \rightarrow\) 433.5, 533.5, 633.5;  
    **합계:** 1600.5.
    
  - **\(d1 = 3\) 총합:** \(1146 + 1600.5 = 2746.5\),  
    **평균:** \(2746.5 / 6 \approx 457.75\).

- **\(d1 = 4\):**  
  - **옵션 A (if \(d2 \le 3\)):**  
    최종 수:  
    \[
    f = 350 + 10 \times 4 + d2 = 390 + d2.
    \]
    예: \(d2 = 1,2,3 \rightarrow\) 391, 392, 393;  
    **합계:** 1176.
    
  - **옵션 B (if \(d2 \ge 4\)):**  
    최종 수:  
    \[
    f = 100 \times d2 + 40 + 3.5 = 100 \times d2 + 43.5.
    \]
    예: \(d2 = 4,5,6 \rightarrow\) 443.5, 543.5, 643.5;  
    **합계:** 1630.5.
    
  - **\(d1 = 4\) 총합:** \(1176 + 1630.5 = 2806.5\),  
    **평균:** \(2806.5 / 6 \approx 467.75\).

- **그룹 2 평균:**  
  \[
  \frac{457.75 + 467.75}{2} = 462.75.
  \]
  사건 수: \(72\)  
  **총합 그룹 2:** \(462.75 \times 72 = 33,\!318\).
...
- **\(d1 = 5\):**  
  - **옵션 A (if \(d2 \le 3\)):**  
    ...
    f = 500 + 35 + d2 = 535 + d2.
    \]
    예: \(d2 = 1,2,3 \rightarrow\) 536, 537, 538;  
    **평균:** \(\displaystyle \frac{536 + 537 + 538}{3} = 537\).
    
  - **옵션 B (if \(d2 \ge 4\)):**  
    최종 수:  
    \[
    f = 500 + 10 \times d2 + 3.5.
    \]
    예: \(d2 = 4,5,6 \rightarrow\) 543.5, 553.5, 563.5;  
    **평균:** 553.5.
    
  - **\(d1 = 5\) 평균:**  
    \(\displaystyle \frac{537 + 553.5}{2} = 545.25\).
...
- **\(d1 = 6\):**  
  - **옵션 A (if \(d2 \le 3\)):**  
    ...
    f = 600 + 35 + d2 = 635 + d2.
    \]
    예: \(d2 = 1,2,3 \rightarrow\) 636, 637, 638;  
    **평균:** 637.
    
  - **옵션 B (if \(d2 \ge 4\)):**  
    최종 수:  
    \[
    f = 600 + 10 \times d2 + 3.5.
    \]
    예: \(d2 = 4,5,6 \rightarrow\) 643.5, 653.5, 663.5;  
    **평균:** 653.5.
    
  - **\(d1 = 6\) 평균:**  
    \(\displaystyle \frac{637 + 653.5}{2} = 645.25\).

- **그룹 3 평균:**  
  \(\displaystyle \frac{545.25 + 645.25}{2} = 595.25\).  
  사건 수: \(72\)  
  **총합 그룹 3:** \(595.25 \times 72 = 42,\!858\).

## 3. 전체 이론적 기댓값

전체 216가지 경우의 총합은
\[
32,\!688 + 33,\!318 + 42,\!858 = 108,\!864.
\]

따라서, 전체 이론적 기댓값은
\[
\frac{108,\!864}{216} = 504.
\]

## 4. 결론

고정 전략과 남은 두 주사위의 최적 배치를 고려한 결과,  
모든 216가지 경우의 최종 세 자리 수의 평균(이론적 기댓값)은 **504**가 됨을 알 수 있습니다.
