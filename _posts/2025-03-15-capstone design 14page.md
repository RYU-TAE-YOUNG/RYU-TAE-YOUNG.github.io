...
예를 들어,  
- 첫 번째 주사위가 3이고, 이를 10의 자리로 사용하기로 결정했다면,  
- 두 번째 주사위가 6이면 최종 숫자는  
  \[
  10 \times 3 + 6 = 36
  \]
  이 됩니다.
...
첫 번째 주사위의 결과를 \(X\), 두 번째 주사위의 결과를 \(Y\)라고 합시다.  
각 전략에 따른 최종 두 자리 수는 다음과 같이 결정됩니다.

- **전략 A (X를 10의 자리에 배치):**  
  최종 숫자 \(= 10X + Y\).  
  두 번째 주사위 \(Y\)는 1부터 6까지 균등하게 나오므로,  
  \(E[Y] = 3.5\).  
  따라서, 전략 A의 기댓값은  
  \[
  10X + 3.5.
...
두 전략 중 어느 쪽이 더 유리한지 비교하려면, 다음 부등식을 풉니다:
...
\[
9X > 31.5 \quad \Longrightarrow \quad X > 3.5.
\]

따라서,

- 만약 **\(X \ge 4\)**라면 전략 A (X를 10의 자리에 배치)가 유리합니다.
- 만약 **\(X \le 3\)**이라면 전략 B (X를 1의 자리에 배치)가 유리합니다.

## 결론

게임에서 이길 가능성을 최대화하려면 다음 전략을 사용해야 합니다:

- **첫 번째 주사위 눈이 4 이상**이면,  
  이를 10의 자리에 배치하여 최종 숫자를 \(10X + Y\)로 만듭니다.
- **첫 번째 주사위 눈이 3 이하**이면,  
  이를 1의 자리에 배치하여 최종 숫자를 \(10Y + X\)로 만듭니다.

이 전략을 사용하면 두 자리 숫자의 기댓값이 최대가 되어, 게임에서 이길 확률이 가장 높아집니다.
```r
